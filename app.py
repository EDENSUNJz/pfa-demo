...
def safe_ranges(schema_cols, ranges):
    ...
    def guess(name):
        ...
        return {"lo": -1.0, "hi": 1.5, "default": 0.0, "step": 0.1}
    out = {}
...
def save_feedback_csv(task, y_pred, label, easy, understand, trust, comment, inputs, export_dir: str):
    try:
        ...
        p = export_path / "feedback_log.csv"
        if p.exists():
            try:
...
for f in sorted(schema_cols):
    ...
    title = bi(f)
    if f.endswith("_yes"):
        user_adv[f] = st.selectbox(
            title, [0, 1], index=int(r.get("default", 0) > 0),
            format_func=lambda x: "是/Yes" if x == 1 else "否/No",
            key=f"adv_{f}"
        )
    elif f.endswith("_z"):
        user_adv[f] = st.slider(
            title, float(r["lo"]), float(r["hi"]), float(r["default"]),
            step=float(r["step"]), key=f"adv_{f}"
        )
    else:
        user_adv[f] = st.number_input(
            title, value=float(r.get("default", 0.0)), step=0.1, key=f"adv_{f}"
        )

if st.button("用专家模式预测 / Predict (Expert)"):
    with st.spinner("计算中 / Working..."):
